...
public class UserDetailServiceImpl implements UserDetailsService, MessageSourceAware{
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException{
		List<UserDetails> users = userDetailDao.loadUsersByUsername(username);
		...
		if (enableAuthorities) 
			dbAuthsSet.addAll(userDetailDao.loadUserAuthorities(user.getUsername()));
//		if (enableGroups) 
...
//		addCustomAuthorities(user.getUsername(), dbAuths);
		...
		if (dbAuths.size() == 0) {
			...
			throw new UsernameNotFoundException(messages.getMessage("UserDetailDAOImpl.noAuthority", new Object[] { username }, "User {0} has no GrantedAuthority"));
		}

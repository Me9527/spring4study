...
public class CustomSecurityMethodMetadataSource extends AbstractFallbackMethodSecurityMetadataSource
...
//		}
    	...
    	SecurityConfig teller = new SecurityConfig(rolePrefix + "teller");
    	SecurityConfig user = new SecurityConfig(rolePrefix + "user");
    	SecurityConfig supervisor = new SecurityConfig(rolePrefix + "supervisor");
    	List<ConfigAttribute> t1 = new ArrayList<ConfigAttribute>();
    	t1.add(teller);	//t1.add(user); t1.add(supervisor);
    	
//    	List<ConfigAttribute> t2 = new ArrayList<ConfigAttribute>();
//    	t2.add(teller);	t2.add(user); 
    	//org.example.module02.services.impl.addBiz01
    	//methodMap.put("org.example.module02.services.addBiz01", t1);
//    	methodMap.put("org.example.module02.services.addBiz01", t2);
    	
    	addSecureMethod("org.example.module02.services.IBizTwo.addBiz01", t1);
	}

	public CustomSecurityMethodMetadataSource() {
	}
	...
	/**
	 * Add configuration attributes for a secure method. Mapped method names can end
	 ...
	 *            applies to
	 * @param mappedName
	 *            mapped method name, which the javaType has declared or inherited
	 * @param attr
	...
	/**
	 * Adds configuration attributes for a specific method, for example where the
	 * method has been matched using a pointcut expression. If a match already
	 * exists in the map for the method, then the existing match will be retained,
	 * so that if this method is called for a more general pointcut it will not
	 * override a more specific one which has already been added.
	 * <p>
	...
	/**
	 * Stores both the Java Method as well as the Class we obtained the Method from.
	 * This is necessary because Method only provides us access to the declaring
	 * class. It doesn't provide a way for us to introspect which Class the Method
	 * was registered against. If a given Class inherits and redeclares a method
	 * (i.e. calls super();) the registered Class and declaring Class are the same.
	 * If a given class merely inherits but does not redeclare a method, the
	 * registered Class will be the Class we're invoking against and the Method will
	 * provide details of the declared class.
	 */
